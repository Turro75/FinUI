				...
				if (total>0) {
					...
					for (int i=top->start,j=0; i<top->end; i++,j++) {
						...
						if (j==selected_row) {
							...
							GFX_truncateText(font.medium, entry_unique, unique_name, available_width, SCALE1(BUTTON_PADDING*2));
						
							SDL_Surface* text = TTF_RenderUTF8_Blended(font.medium, unique_name, COLOR_DARK_TEXT);
							...
							GFX_truncateText(font.medium, entry_name, display_name, available_width, SCALE1(BUTTON_PADDING*2));
						}
						SDL_Surface* text = TTF_RenderUTF8_Blended(font.medium, display_name, text_color);
						SDL_BlitSurface(text, &(SDL_Rect){
							0,
							0,
							max_width-SCALE1(BUTTON_PADDING*2),
